...
type streamPlayer = {
  next: unit => iteratorResult<streamEvent>,
  movesPlayed: unknown,
  done: bool,
}
...
let parse: string => program = %raw(`_parse`)
let importAlg: string => importedProgram = %raw(`_importAlg`)
let measure: (program, option<string>) => measurement = %raw(`_measure`)
let streamEvents: (program, option<string>) => 'generator = %raw(`_streamEvents`)
let stream: (program, option<string>) => 'generator = %raw(`_stream`)
let unfold: (program, int, option<string>) => array<streamEvent> = %raw(`_unfold`)
let prefix: (program, int, option<string>) => array<string> = %raw(`_prefix`)
let createStreamPlayer: (program, option<string>) => streamPlayer = %raw(`_createStreamPlayer`)
let window: (program, unknown, int, option<string>) => array<string> = %raw(`_window`)
