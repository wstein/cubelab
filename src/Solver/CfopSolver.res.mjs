...
function rotatedMacroActions(solved, sources) {
  let output = [];
...
function advancedCornerTriggers(solved) {
  return rotatedMacroActions(solved, [
    "R U R'",
    "R U' R'",
    "R U2 R'",
    "R' U' R",
...
function planLayerPiecesFrom(state, pieces, completed, baseCorners, baseEdges, corners, atomics, triggers, labels, failed) {
  ...
  pieces.forEach((piece, labelIndex) => {
    ...
    let found = BeginnerSolver.searchAtomicWithLimit(state, targetCorners, targetEdges, rankedActions(atomics), 9, 40000);
    if (found === undefined) {
      return;
    }
    let alg$1 = MoveTransform.rotate(BeginnerSolver.flattenActions(found), "X", 2);
...
function findLayerPlanPieces(state, pieces, baseCorners, baseEdges, corners, atomics, triggers, labels) {
...
function planLayerPieces(state, pieces, baseCorners, baseEdges, corners, atomics, triggers, labels) {
  let plan = findLayerPlanPieces(state, pieces, baseCorners, baseEdges, corners, atomics, triggers, labels);
  if (plan !== undefined) {
    return [
      plan,
...
function planMiddleLayerPieces(state, solved, baseCorners, baseEdges, pieces, labels) {
  let actions = BeginnerSolver.downTurns(solved).concat(BeginnerSolver.macroVariants(solved, "U R U' R' U' F' U F")).concat(BeginnerSolver.macroVariants(solved, "U' L' U L U F U' F'"));
  let current = state;
  let plan = [];
  for (let offset = 0, offset_finish = pieces.length; offset < offset_finish; ++offset) {
    let targets = baseEdges.concat(Belt_Array.makeBy(offset + 1 | 0, index => pieces[index]));
    ...
    let path = BeginnerSolver.searchMacros(current, actions, goal, projection, 5);
    let path$1;
    if (path !== undefined) {
      path$1 = path;
    } else {
    ...
    let alg = MoveTransform.rotate(BeginnerSolver.flattenActions(path$1), "X", 2);
    plan.push({
      pair: pieces[offset],
      path: path$1,
      score: physicalCost(alg).total,
      label: labels[offset]
    });
    current = applyPath(current, path$1);
  }
  return [
    plan,
    current
...
function solveAdvancedLbl(input) {
  try {
    ...
    let match$1 = planLayerPieces(current, [
      ...
      2,
      3
    ], true, atomics, cornerTriggers, cornerNames);
    ...
    let match$2 = planMiddleLayerPieces(current, solved, [
      0,
      1,
      2,
      3
...
export {
  ...
  layerPlanKey,
  rotatedMacroActions,
  advancedCornerTriggers,
  triggerLayerCandidate,
  planLayerPiecesFrom,
  findLayerPlanPieces,
  planLayerPieces,
  planMiddleLayerPieces,
  solveLevel,
  solveAdvancedLbl,
  solveBeginner,
  solveFull,
