...
test("the 2x2 through 5x5 manual state editor keeps a constrained draft separate from Setup", () => {
  ...
  assert.match(styles, /\.manual-state-stage\s*\{[^}]*overflow-x:\s*visible;[^}]*overflow-y:\s*clip;/);
  assert.ok(
  ...
  assert.match(client, /const hasSetupState = setup\?\.TAG === "Ok"[\s\S]{0,300}: resetManualState\(manualSize\)[\s\S]{0,400}if \(!hasSetupState\) refreshManualStateAutoFill\(manualSize, true\)/);
  ...
  assert.match(styles, /data-representation="open-cube"[^}]*data-face="B"\]\s*\{[^}]*left:\s*calc\(5\.5% - var\(--manual-state-face-gap\) \* 0\.225\);[^}]*top:\s*calc\(100% \* 0\.95 \/ 2\.4\);[^}]*width:\s*25\.5%;[^}]*height:\s*calc\(100% \/ 2\.4\);[^}]*transform:\s*translateY\(-50%\)/);
...
test("the web UI explains source portability without claiming competition legality", () => {
  ...
  assert.match(page, /data-compatibility-profile="jaap"/);
