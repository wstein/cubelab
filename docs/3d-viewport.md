...
- **Orientation marker:** the compact x/y/z overlay is a viewport-frame Right/Up/Front legend: its `x`, `y`, and `z` arrows stay aligned to those screen-facing directions while colours identify the cube centres currently at R/U/F. A confirmed whole-cube `x/y/z` regrip contracts the glyph, atomically swaps to the normalized centre-colour frame, then expands.
