...
- **Orientation marker:** the compact x/y/z overlay follows live gyro orientation in the same direction as the displayed cube. Explicit whole-cube `x/y/z` moves counter-rotate the matching glyph axis, including confirmed virtual regrips and in-flight Play-mode animations. On a confirmed regrip the glyph contracts, swaps atomically to the normalized orientation and refreshed centre colours, then expands; labels stay fixed as R/L=`x`, U/D=`y`, F/B=`z`.
