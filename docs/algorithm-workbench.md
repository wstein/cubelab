...
For the live viewport it adopts the nearest non-identity cardinal pose after three
samples; it has no separate angular acceptance threshold. It rebases after each accepted
pose rather than treating an early threshold crossing as a quarter turn.
