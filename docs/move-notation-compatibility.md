...
| [ACube 4](https://github.com/josef-jelinek/acube) | 3×3 positional/cycle state input and turn-list spelling | Covered for complete states and executable turns | Setup imports exact positional states, cycles, and `+`/`-` orientation terms. ACube partial constraints (`?`, `@`, and bracketed ignored pieces) intentionally remain unresolved because CubeLab requires one physical state. In the ACube dialect, lowercase `e/s/m` map to `y'/z/x'`; `R*` allowed-turn lists are solver configuration, not algorithms. |
